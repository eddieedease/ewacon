...
$app->get('/archive/{arcadeid}', function (Request $request, Response $response) {
    ...
    $dbh = new PDO("mysql:host=$hostname;dbname=$db_name", $username, $password);
    $sqlhs = "SELECT * FROM highscores WHERE linkid = '$legacyname'";
	...
	$legacyHighScoresArray = array(
        "game1" => json_decode($legacygame1score),
        "game2" => json_decode($legacygame2score),
        "game3" => json_decode($legacygame3score),
        "game4" => json_decode($legacygame4score)
    );
    // Encode for putting in database
